...
st.markdown("""
    <style>
       # Create visualization    # Create visualizations
    # Display charts
    ...
    st.altair_chart(stacked_chart, use_container_width=True) charts
    st.subheader("Container Volume Analysis")eader {
            font-size: 2.5rem;
            color: #1f77b4;
            text-align: center;
            margin-bottom: 2rem;
...
def create_stacked_chart(df_melted, selected_companies, period_labels):
    ...
    # Add grand total labels on top of bars
    grand_total_labels = alt.Chart(grand_totals).mark_text(
        ...
        y=alt.Y('Total throughput:Q'),
        text=alt.Text('Total throughput:Q', format='.1f')
    )
